...
Capsule разделяет «что показать», «как реагировать» и «что делать с миром» по разным физическим слоям. Entity рендерит JSX и не знает ничего про state, API или роутинг. Controller — FSM на XState, который перехватывает события у потомков через Proxy. Feature держит side-effects. Widget склеивает всё это композицией. На границах работают линтер (`@capsuletech/compliance`) и кодген (`@capsuletech/vite`), которые не дают сломать регламент.
...
| Build | Vite 6 + кастомные плагины (`@capsuletech/vite`) |
| Styling | Tailwind v4 + CVA + `@capsuletech/style` |
...
`@capsuletech/ui` строит компоненты поверх Kobalte (доступность, behavior) и CVA (варианты). `@capsuletech/style` даёт `createStyle` для семантических токенов, тем и slot-overrides. Tailwind v4 — как rendering-engine для классов. Темизация и кастомизация — first-class, а не post-hoc патч.

→ [@capsuletech/ui](docs/09-packages/ui.md) · [@capsuletech/style](docs/09-packages/style.md)
...
→ [@capsuletech/core](docs/09-packages/core.md) · [@capsuletech/state](docs/09-packages/state.md) · [@capsuletech/router](docs/09-packages/router.md) · [@capsuletech/ui](docs/09-packages/ui.md) · [@capsuletech/style](docs/09-packages/style.md) · [@capsuletech/compliance](docs/09-packages/compliance.md) · [api-middleware](docs/09-packages/api-middleware.md)
