...
Capsule разделяет «что показать», «как реагировать» и «что делать с миром» по разным физическим слоям. Entity рендерит JSX и не знает ничего про state, API или роутинг. Controller — FSM на XState, который перехватывает события у потомков через Proxy. Feature держит side-effects. Widget склеивает всё это композицией. На границах работают линтер (`@capsule/compliance`) и кодген (`@capsule/vite`), которые не дают сломать регламент.
...
- **Auto-import без импортов.** `Page`, `Widget`, `Entity`, `Controller`, `Feature` — глобальные wrappers. Слоты приходят позиционными аргументами с типизацией через сгенерённый `CapsuleSlots`.
...
| Build | Vite 6 + кастомные плагины (`@capsule/vite`) |
| Styling | Tailwind v4 + CVA + `@capsule/style` |
...
```bash
nx g @nx/js:library --name=@capsule/X --directory=packages/X \
  --importPath=@capsule/X --publishable --buildable
```
...
`@capsule/ui` строит компоненты поверх Kobalte (доступность, behavior) и CVA (варианты). `@capsule/style` даёт `createStyle` для семантических токенов, тем и slot-overrides. Tailwind v4 — как rendering-engine для классов. Темизация и кастомизация — first-class, а не post-hoc патч.

→ [@capsule/ui](docs/09-packages/ui.md) · [@capsule/style](docs/09-packages/style.md)

### Пакеты

Монорепо `@capsule/*`. Юзер на уровне приложения держит в зависимостях только Capsule-пакеты; solid-js / vite / xstate / @tanstack — скрытые внутренние детали.

→ [@capsule/core](docs/09-packages/core.md) · [@capsule/state](docs/09-packages/state.md) · [@capsule/router](docs/09-packages/router.md) · [@capsule/ui](docs/09-packages/ui.md) · [@capsule/style](docs/09-packages/style.md) · [@capsule/compliance](docs/09-packages/compliance.md) · [api-middleware](docs/09-packages/api-middleware.md)
