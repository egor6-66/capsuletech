...
- `CLAUDE.md` — обновить раздел Desktop под новый flow (`capsule desktop dev <app>` вместо `pnpm desktop <app>`).
...
| 5 | CLI command | owner-cli | `capsule desktop dev/build <app>` action импортирует `@capsuletech/desktop` напрямую (вместо `execa scripts/desktop.mjs`) |
| 6 | Verdaccio + smoke | owner-tests | `@capsuletech/desktop` в `nx.json:release.groups.cli`. Smoke в `capsule-test`: `capsule desktop dev sandbox` → Tauri окно открывается + devUrl connects |
...
- `pnpm desktop sandbox` → `capsule desktop dev sandbox` (новый UX).
...
- **Hard breaking change** для framework-developer workflow: `pnpm desktop sandbox` → `capsule desktop dev sandbox`.
