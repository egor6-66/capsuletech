...
export const buildAppAction: CommandAction = async (ctx) => {
  if (ctx.type !== 'app' || !ctx.root) {
    kit.log.error('Build запускается только внутри apps/<name>/');
    return;
  }
  ...
  const fn = viteActions.buildCapsuleApp as
    | ((cfg: unknown, appRoot: string, wsRoot: string) => Promise<unknown>)
    | undefined;
  if (!fn) {
    kit.log.error('shared-vite не экспортирует buildCapsuleApp');
    return;
  }
