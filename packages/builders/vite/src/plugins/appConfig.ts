 ...
 *
 * Статический `import { endpoints } from './registry/endpoints'` тут безопасен
 * после удаления `dirs:` из AutoImport (см. capsuleConfig.ts). Раньше AutoImport
 * сканировал .capsule/registry/, инжектил `endpoints` как глобал и тянул
 * registry/endpoints.ts в createApi.ts → циклическая evaluation web-query.
 * Сейчас никаких inject'ов нет, цепочка app-config.gen → endpoints → auth.ts
 * → web-query идёт ровно один раз и без TDZ.
 */
