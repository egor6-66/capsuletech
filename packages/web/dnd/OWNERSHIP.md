...
- **Window-level listeners reset** — Не забыть `cleanup()` вызвать с `removeEventListener` на все 4 listener'а: pointermove, pointerup, pointercancel, keydown. Иначе зависнут в памяти при repeat-drag'ах. See `context.tsx:235-247`.
...
| Unit | (none yet) | API contracts, findDroppableAt, state transitions |
...
**Перед изменением:** unit-tests должны быть green (`pnpm --filter @capsuletech/web-dnd test`). Сейчас нет unit-тестов web-dnd; Matrix integration tests покрывают основной flow.
